...
import random,extract_gif,mywc,random_deform,pic2pic,mymath,myGeometry,math,lumin2d,myio
import numpy as np
...
def cmd_word_cloud(ctx):
	...
	def f(ctx):
		...
		wcpic=mywc.wordCloud(img,wordFrequencies=wordFrequencies,wordMax=500).convert("RGB")
...
def cmd_zoomin_blur(ctx):
	...
	def f(ctx):
		...
		for i in range(len(imgs) if is_gif else 10):
			if(is_gif):
				_.append(zoomin_blur(imgs[i].convert("RGB"),dist=random.random()*0.3))
			else:
				_.append(zoomin_blur(img.convert("RGB"),dist=random.random()*0.3))
...
def cmd_random_deform(ctx):
	...
	def _f(ctx=ctx):
		...
		imgs=[random_deform.random_deform(img,cnt=500,x_range=0.0,y_range=0.0,neibour_cnt=3) for _ in range(5)]
	...
	for i in range(-4,5):
		...
		for x in range(w):
			for y in range(h):
				...
				temp=temp*i*w/3
				qd[(x,y)]=(npa((x-temp,y)))
		imgs.append(random_deform.random_deform(img,cnt=0,neibour_cnt=3,qd=qd,method=0))
	...
	for t in range(-1,5):
		...
		for i in range(w):
			for j in range(h):
				...
				qd[pp.xy]=npa((i,j))
		imgs.append(random_deform.random_deform(img,cnt=0,neibour_cnt=4,qd=qd,method=0))
	...
	for frame in range(10):
		...
		for i in range(len(imgs)):
			for j in range(len(imgs[i])):
				...
				left=int(lefts[i][j]+(random.random()-random.random())*border)
...
def mb(ctx,match,rest):
	...
	for i in range(frames):
		...
		img_=img_.rotate(random.random()*36*rrr-18*rrr,fillcolor=pic2pic.get_border_color(img_))
		...
		r1=r//2
		angle1=angle
	gif=make_gif(imgs,ss=2200000//frames,fps=int(fps),pth=path.join(mainpth,'temppic'))
	simple_send(ctx,gif)
...
def cmd_lumin(ctx,match,rest):
	...
	v1=np.array(lumin2d.illuminate(pic,(0,1,0.5),vn=vn,gr=gr,func_newc=f)).astype(np.float32)
	v2=np.array(lumin2d.illuminate(pic,(1,0,0.5),vn=vn,gr=gr,func_newc=f)).astype(np.float32)
	v3=np.array(lumin2d.illuminate(pic,(0,-1,0.5),vn=vn,gr=gr,func_newc=f)).astype(np.float32)
	v4=np.array(lumin2d.illuminate(pic,(-1,0,0.5),vn=vn,gr=gr,func_newc=f)).astype(np.float32)
	rets=[]
	for i in range(10):
		a=i/10
		v=v1*(1-a)+v2*a
...
def cmd_rotate(ctx):
	...
	cuberoot=lambda x:abs(x**(1/3))*(x/abs(x)) if x!=0 else 0
	fuc=lambda i:cuberoot(i-fnum/2)
	d=fuc(fnum)-fuc(0)
	d1=-fuc(0)/d*360
	fuc1=lambda i:cuberoot(i-fnum/2)/d*1080+d1
	for i in range(fnum):
		p=pics[i%len(pics)]
		#print(fuc1(i))
...
def cmd_pet(ctx,match,rest):
	...
	def pet(img,pth=pth,size=720,ss=5e6,high_frame_rate=False):
		...
		imgs=[Image.open(_).resize((size,size)) for _ in glob(path.join(pth,'*.png'))]
		if(high_frame_rate):
...
def cmd_font(ctx):
	...
	p=txt2im_ml(text,font=fnt,width=512,fixedHeight=36,trim_width=True,bg=(255,)*4)
	simple_send(ctx,p)
@receiver
...
def cmd_bend_gif(ctx):
	...
	if(rpic[-3:]=='gif'):
		...
		fps=18
	framenum=len(frames)
	frames=[pic2pic.im_sizeSquareSize(im,2e6/min(framenum,200)) for im in frames]
	if(framenum>200):
		rate=framenum/200
	...
	ret=[]
	ft=lambda t:sin(t*_2pi*3)
	fyt=lambda y,t:(ft(t)-0.5)*(y-height/2)/12+width/9*sin(ft(t)*_2pi+y/height*(4+ft(t)))
	def get(im,x,y):
...
def cmd_makeqr(ctx,match,rest):
	...
	bsiz=myqr.boxsiz
	w,h=qr.size
	inb=lambda x,y,le,ri,up,lo:(le<=x<=ri)and(up<=y<=lo)
	dots=0.2
	for x in range(w):
		for y in range(h):
...
def cmd_grou_pintu(ctx,match,rest):
	...
	if(sctx.get_rpics()):
		...
		bg=getter.get_image(r'http://p.qlogo.cn/gh/%s/%s/0'%(gid,gid)).convert("RGB")
	memblist=_bot.sync.get_group_member_list(self_id=int(sctx.self_id),group_id=int(sctx.group_id))
	imgs=[]
	uids=[]
	siz=20
