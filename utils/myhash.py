...
class splitedDict:
    ...
    def dumpPart(self,savepth,name):
        ...
        tmp=path.join(savepth,"%s.json"%sk)
        myio.dumpjson(tmp,self.dic.get(sk,{}))
    def load(self,savepth):
        print('load!!!',savepth)
        for filename in glob(path.join(savepth,'*.json')):
    ...
    def __contains__(self,name):
        ...
        if(not(sk in self.dic)):
            ...
            return True
    def part_savepth(self,sk):
        return path.join(self.pth,"%s.json"%sk)
